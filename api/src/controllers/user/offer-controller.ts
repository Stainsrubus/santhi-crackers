...
.get(
  ...
  async ({ set, store }) => {
    ...
    try {
      ...
      const [flatOffers, negotiateOffers, discountOffers, mrpOffers] = await Promise.all([
        FlatOffer.find().populate('items.productId').lean(),
      ...
      // Add favorite status to items and filter out inactive items
      const processedOffers = offers.map((offer) => {
        ...
        if (offer.items && offer.items.length > 0) {
          //@ts-ignore
          processedOffer.items = offer.items
            .filter((item: { active: boolean; }) => item.active !== false) // Filter out inactive items
            .map((item: { _id: { toString: () => any; }; productId: { _id: { toString: () => string; }; }; }) => {
      ...
      }).filter(offer => offer.items && offer.items.length > 0); // Filter out offers with no active items
  ...
  },
  {
    detail: {
      ...
      description: "Fetches all offers (flat, negotiate, discount, mrp) with populated product details and favorite status based on user ID. Filters out inactive items.",
    },
  },
)
.get(
