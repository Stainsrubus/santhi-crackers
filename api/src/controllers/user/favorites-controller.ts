  ...
  .get(
    ...
    async ({ store }) => {
      ...
      try {
        ...
        // Flatten all products and filter out those with inactive categories
        const products = favorites.flatMap((fav: any) =>
          fav.products?.filter((p: any) => p?.category?.active)
        );
  
        return {
          message: "Favorites product count fetched successfully",
          status: true,
    ...
    {
      detail: {
        summary: "Get count of favorite products",
        description: "Returns total number of products in a user's favorites with active categories only",
      },
    }
  )  
