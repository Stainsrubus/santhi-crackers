<script>
     ...
     * @property {{_id: string, name: string}} category
     ...
     * @property {{_id: string, name: string} | null} brand
     * @property {Array<{name: string, fields: Record<string, string>}>} specifications
     * @property {Array<{title: string, values: string[]}>} options
     * @property {number} stock
     * @property {number} onMRP
     * @property {number} flat
     * @property {boolean} favorite
     * @property {number} gst
     * @property {number} negotiateLimit
     * @property {boolean} negotiate
     * @property {number | null} negMOQ
     */
    ...
    // Reactive productId assignment
    $: productId = $page.params.id;
    $: console.log('Reactive: productId updated to:', productId);
  
    // Force query refetch when productId changes
    $: if (productId && $productQuery) {
      console.log('Reactive: Forcing refetch for productId:', productId);
      $productQuery.refetch();
    }
  
    onMount(() => {
      console.log('onMount: productId is:', productId);
      if (productId && $productQuery) {
        console.log('onMount: Forcing refetch for productId:', productId);
        $productQuery.refetch();
      }
      ...
      queryFn: async () => {
        ...
        try {
          ...
          return {
            ...
            negotiateLimit: product.negotiateLimit || 0,
            negotiate: product.negotiate || false,
    ...
    $: visibleSpecs = showAllSpecs && $productQuery?.data ? 
      $productQuery.data.specifications : 
      ($productQuery?.data?.specifications || []).slice(0, 2);
  
    $: discountPercentage = $productQuery?.data ? 
      Math.round((($productQuery.data.strikePrice - $productQuery.data.MRP) / $productQuery.data.strikePrice) * 100) : 0;
  </script>
        ...
        <div class="flex flex-col-reverse lg:flex-row gap-4">
          ...
          <div class="flex-1">
            <div class="relative aspect-square lg:w-[400px] md:w-[280px] w-full h-full md:h-[280px] lg:h-[400px] rounded-2xl overflow-hidden bg-gray-100 border">
              ...
              {:else if $productQuery?.data?.images?.length > 0}
                <img
            ...
            <div class="flex gap-3 mt-6">
              ...
              <button 
                ...
                class="flex-1 bg-custom-gradient  hover:shadow-lg text-white py-3 px-6 rounded-lg hover:scale-95 duration-500  transition-all flex items-center justify-center gap-2 font-medium disabled:bg-gray-400 disabled:cursor-not-allowed"
            ...
            <!-- <div class="grid grid-cols-3 gap-4 mt-6 p-4 bg-gray-50 rounded-lg">
              <div class="text-center">
                <Truck class="w-6 h-6 text-green-600 mx-auto mb-2" />
                <p class="text-xs text-gray-600">Free Delivery</p>
              </div>
              <div class="text-center">
                <Shield class="w-6 h-6 text-blue-600 mx-auto mb-2" />
                <p class="text-xs text-gray-600">2 Year Warranty</p>
              </div>
              <div class="text-center">
                <RefreshCw class="w-6 h-6 text-orange-600 mx-auto mb-2" />
        ...
        {:else if $productQuery?.data}
          ...
          <div>
            ...
            <div class="flex items-center gap-4 mb-4 flex-wrap">
              ...
              {/if}
              {#if $productQuery.data.strikePrice > $productQuery.data.MRP}
                <span class="text-xl text-gray-500 line-through">
                  ₹{$productQuery.data.strikePrice.toLocaleString()}
                </span>
              {/if}
