<script lang="ts">
	...
	async function fetchManagers(limit = 10, page = 1, search = '') {
		const res = await _axios.get(`/manager/all?limit=${limit}&page=${page}&q=${search}`);
	...
	const query = createQuery({
		...
		queryFn: () => fetchManagers(limit, page, search)
	});

	const deleteMutation = createMutation({
		mutationFn: ({ id, permanent }: { id: string; permanent: boolean }) =>
			_axios.delete(`/manager/${id}?permanent=${permanent}`),
		onSuccess({ data }) {
			queryClient.refetchQueries({
				queryKey: ['manager fetch']
			});
			goto('/hidden-admin-base-007/dashboard/managers?mode=list');
			toast(data?.message ?? 'Manager Edited');
			modelOpen = false;
		},
	...
	<div class="mt-6 text-maintext w-[calc(100vw-420px)] font-pt mx-auto">
		...
		<Table.Root class="">
			...
			{:else if $query?.data?.total === 0}
				<TableCaption class="text-center w-full text-xs">No Managers Found!</TableCaption>
			{/if}
			<Table.Header>
				<Table.Row class="">
			...
			<Table.Body>
				{#each $query.data?.managers || [] as manager, i}
					<Table.Row>
						<Table.Cell>{i + 1}</Table.Cell>
						<Table.Cell>{manager.name}</Table.Cell>
						<Table.Cell>{manager.email}</Table.Cell>
						<Table.Cell class="flex items-center"
							>{formatDate(new Date(manager.createdAt))}</Table.Cell
						>
						<Table.Cell>
							<Switch
								class="text-right"
						...
						<Table.Cell class="flex gap-2">
							<button
								onclick={() => {
									$managerStore = {
										...
										joinedAt: formatDate(new Date(manager.createdAt))
									};
								}}
							>
							...
							<button onclick={() => ((modelOpen = true), (selectedManagerId = manager._id))}>
			...
			<Dialog.Content class="p-6">
				...
				<div class="flex gap-4 justify-around">
					<Button
						...
						onclick={() => $deleteMutation.mutate({ id: selectedManagerId, permanent: true })}
						>Yes</Button
					>
